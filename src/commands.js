...
export async function cmdBake(args) {
  ...
  writeFileSync(join(out, "asset-portal-manifest.json"),
    JSON.stringify({ project, baked_at: new Date().toISOString(), assets: baked }, null, 2));
  writeFileSync(join(out, "AssetPortalBaked.swift"), genSwiftAccessor(baked, folderName));

  console.log("");
  console.log(c.green(`✓ Baked ${baked.length} file(s)`) + ` + asset-portal-manifest.json + AssetPortalBaked.swift`);
  console.log(c.dim(`  Add the "${folderName}" folder to your app target. In production, use AssetPortalBaked`));
  console.log(c.dim(`  (no AssetPortalKit dependency) — see swift/AssetPortalKit/README.md.`));
...
function genSwiftAccessor(baked, folderName) {
  ...
  return `// Generated by \`asset-portal bake\`. Do not edit.
//
// Zero-dependency accessor for assets baked into the app bundle. Use this in
// production INSTEAD of AssetPortalKit, so the over-the-air downloader is not
// compiled into release builds, e.g.:
//
//   #if DEBUG
//   let url = portal.localURL(forKey: "hero_ship")   // AssetPortalKit
//   #else
//   let url = AssetPortalBaked.url(forKey: "hero_ship")
//   #endif
import Foundation

public enum AssetPortalBaked {
